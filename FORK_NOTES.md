...
|---|---|---|---|---|
| 2026-09-12 | 15 commits, `b764c4a` through `5475934` — upstream's **inference engine**: a new `extensions_built_in/inference_engine/` extension (persistent GPU-resident engine + socket protocol + latent preview), a `/generate` page with LoRA browser/toggle and lokr support, `api/inference/[...path]`, `api/loras`, `api/jobs/[jobID]/kill`, `toolkit/{inference_lora,sample_step_hook,ui_job_status}.py`, `toolkit/models/registry.py` + `models/v2/pool.py`; plus a HiDream x0-prediction/custom-weighting change, a forced-pure-noise-step config, an EMA-feedback scale fix, DFE cleanup, and two version bumps to `0.13.8` | 15 adopted as-is, 0 rejected | **Zero textual conflicts** — 52 files, +6952/−37, all additive or in upstream-owned files. One divergence repair, the exact recurrence this table's Next 15 param-type row predicts: upstream's new `ui/src/app/api/jobs/[jobID]/kill/route.ts` typed `params` as the sync Next 14 shape while `await`ing it, so `tsc` would have failed once the fork's stricter checking ran; re-typed to `Promise<{ jobID: string }>`. That takes the merge surface from **57 to 58** modified upstream files (`EXPECTED_TOUCHPOINTS` and the handler-count row updated in the same commit). `verify_fork`'s next-params regex was also tightened: it matched axios request options (`{ params: { id: startingJobId } }`) in upstream's new `generate/page.tsx` as if they were route params — two false FAILs. It now requires a primitive type after the inner key, so a value expression no longer trips it. The three conflict-prone JSX mounts, the fork docs registry, and the preset list all survived untouched | Full `run_fork_tests.ps1`: **12/12 gates PASS, zero SKIPs** — `verify_fork`, six Python suites (`test_dataset_selection`, `test_fork_speed`, `test_ideogram4_prompt`, `test_lora_compile_scalars`, `test_qol_scripts`, `test_presets`), `py_compile` on the fork scripts, `npm test` (32 Node contract tests), `tsc --noEmit` **0 errors**, worker `tsc`, and `next build` clean. `npm ci` on an upstream-identical lockfile; `restore_ui_pkg.ps1 -Check` confirmed the lockfile byte-identical to upstream and the fork's single `test` script line intact. `py_compile` also run on all 13 new upstream Python files: PASS. Not covered: the engine's socket/GPU runtime path and the cron worker (no GPU, no live server in this session) |
