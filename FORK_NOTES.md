...
|---|---|---|---|---|
| 2026-09-11 | 1 commit, `f56b5a1` — HiDream O1 changed to a proper x0-prediction: `hidream_o1_model.py` drops the flow-matching velocity conversion and returns the model's x0 prediction directly, `get_loss_target` returns raw latents instead of `noise * noise_scale - latents`, `save_model` unifies the dequantize/state-dict-clean path for both the comfy and diffusers save branches, and `default_weighing_scheme.py` gains a large new x0-specific weighting table; `SDTrainer.py` and `custom_flowmatch_sampler.py` pass the model's new `x0_pred` flag into `get_weights_for_timesteps`. 5 files, +1035/-20 | 1 adopted as-is, 0 adopted with divergence work, 0 rejected | Zero textual conflicts (git auto-merged `SDTrainer.py`). `extensions_built_in/sd_trainer/SDTrainer.py` is a fork touchpoint; the new `x0_pred=self.sd.x0_pred` kwarg lands at the timestep-weight call (~line 967), disjoint from the fork's `loss_sync_every`-gated `neutralize_nonfinite_loss`/`DeferredLossTracker` block (~line 2284+), re-verified present by grep after the merge, not assumed. `hidream_o1_model.py`, `custom_flowmatch_sampler.py`, `default_weighing_scheme.py`, and `ui/src/app/jobs/new/options.tsx` are not fork touchpoints (`options.tsx` diffs byte-identical against `upstream/main` post-merge, consistent with its retirement note below). Fork surface unchanged at **57** modified upstream files | `npm ci`, `npx tsc --noEmit` **0 errors**, `npx next build` clean (all routes emitted), `npm test` **62 passed / 0 failed** (identical to baseline), `py_compile` on all 4 touched Python files: PASS. Repo-wide grep for conflict markers: none; working tree clean. Author/committer scan on the merge range: only `socrasteeze <socradeez@gmail.com>` (merge) and `Jaret Burkett <jaretburkett@gmail.com>` (upstream, preserved). **Not covered:** no `.venv`/torch in this Linux container, so `test_dataset_selection`, `test_fork_speed`, `test_ideogram4_prompt`, `test_lora_compile_scalars`, `test_presets` did not run — pre-existing gap; the HiDream x0-prediction switch and its new weighting table are exactly the kind of change those would most want to exercise on real hardware (untested here beyond syntax and the disjoint-touchpoint check); the cron worker's runtime paths remain untested as always |
| 2026-09-10 | 1 commit, `b1bf3e4` — a new optional `first_timestep_chance` config knob: per-sample chance to force the full-noise timestep index (0) instead of the normally-sampled one, defaulting to `0.0` (no behavior change unless set). 2 files, +8/-0 | 1 adopted as-is, 0 adopted with divergence work, 0 rejected | Zero textual conflicts. `toolkit/config_modules.py` is a fork touchpoint; the new `first_timestep_chance` field sits disjoint from the fork's own insertions there. `jobs/process/BaseSDTrainProcess.py` is not a fork touchpoint. Fork surface unchanged at **57** modified upstream files | `py_compile` on both touched files: PASS. `scripts/verify_fork.py`: insertions/touchpoints/next-params/fork-docs/presets all OK. Repo-wide grep for conflict markers: none; working tree clean. Author/committer scan on the merge range: only `socrasteeze <socradeez@gmail.com>` (merge) and `Jaret Burkett <jaretburkett@gmail.com>` (upstream, preserved). **Not covered:** no file under `ui/` changed in this window, so the `npm ci`/`tsc`/`next build`/`npm test` gates were skipped as exercising nothing new; no `torch`/`pytest` in this Linux container, so `test_dataset_selection`, `test_fork_speed`, `test_ideogram4_prompt`, `test_lora_compile_scalars` did not run — pre-existing gap in this environment; `pwsh` is also unavailable here so `scripts/run_fork_tests.ps1`/`restore_ui_pkg.ps1` were not invoked (not needed — no `ui/` package files changed) |
| 2026-09-09 | 2 commits, `b764c4a` through `4a41f14` — DFE cleanup (tightened scale handling in `diffusion_feature_extraction.py`/`tipsv2.py`) followed by a fix so an EMA running with feedback uses its own scale instead of borrowing another's (`toolkit/ema.py`, plus one new field in `config_modules.py` and one new call site in `BaseSDTrainProcess.py`). 5 files, +13/-10 | 2 adopted as-is, 0 adopted with divergence work, 0 rejected | Zero textual conflicts. `toolkit/config_modules.py` and `jobs/process/BaseSDTrainProcess.py` are both fork touchpoints and were re-verified rather than assumed: the `compile_mode`/`compile_fullgraph`/`compile_dynamic` keys and the fork's own insertions in `BaseSDTrainProcess.py` are intact and disjoint from upstream's one-line addition to each. `toolkit/ema.py`, `toolkit/models/diffusion_feature_extraction.py`, and `toolkit/models/tipsv2.py` are not fork touchpoints and are byte-identical to `upstream/main` after the merge. Fork surface unchanged at **57** modified upstream files | `py_compile` on all 5 touched files: PASS. Repo-wide grep for conflict markers: none; working tree clean. Author/committer scan on the merge range: only `socrasteeze <socradeez@gmail.com>` (merge) and `Jaret Burkett <jaretburkett@gmail.com>` (upstream, preserved). **Not covered:** no file under `ui/` changed in this window, so the `npm ci`/`tsc`/`next build`/`npm test` gates were skipped as exercising nothing new; no `torch` in this container, and `download.pytorch.org` is not reachable through this environment's proxy allowlist, so `test_dataset_selection`, `test_fork_speed`, `test_ideogram4_prompt`, `test_lora_compile_scalars` did not run — pre-existing gap, and the EMA feedback-scale fix is exactly the kind of change those would most want to exercise on real hardware |
| 2026-09-08 (2nd) | 2 commits, `af82197` through `fb7c14b` — the Sapiens DFE now enables gradient checkpointing and uses it whenever gradients are enabled even though the feature extractor stays in eval mode, followed by the `0.13.6` version bump. 3 files, +4/-2 | 2 adopted as-is, 0 adopted with divergence work, 0 rejected | Zero textual conflicts. `toolkit/models/diffusion_feature_extraction.py`, `toolkit/models/sapiens2.py`, and `version.py` are upstream-owned and outside the fork merge surface. The preceding DFE rework already present on `origin/main` was reconciled before this window; no fork insertion point moved. `scripts/verify_fork.py` re-derived the fork surface at **57** modified upstream files, and `scripts/restore_ui_pkg.ps1 -Check` confirmed the fork test script plus an upstream-identical lockfile | Pre-merge and post-merge `scripts/run_fork_tests.ps1` runs were identical and fully green: `verify_fork`, all six Python suites, fork-script `py_compile`, `npm test` **62 passed / 0 failed**, UI and worker TypeScript checks, and `next build`. The 3 incoming Python files also byte-compiled directly. Repo-wide exact conflict-marker scan: none. **Not covered:** the Sapiens DFE checkpointing path requires a real training run and GPU; the cron worker runtime and Dataset Tools model-download paths remain outside the gate script |
| 2026-09-08 | 2 commits, `af89a27` through `3ab6eaa` — a torchao partial-offload fix (`MemoryManager._move_layer_params_to_device` now walks quantized tensor subclasses via `__tensor_flatten__` to find their real storage device and swaps the whole `Parameter` when it's an `AffineQuantizedTensor`, since `.data = ...` on a quantized wrapper only moves the outer metadata and leaves the inner storage behind — two new helpers `_storage_device`/`_move_own_param` in `manager_modules.py`), and an FFT loss logging addition (`SDTrainer.py`'s `do_fft_velocity_equiv_weight` branch now means `fft_loss` once and logs it to `additional_logs['loss/fft']` before adding it into `additional_loss`, instead of adding the unlogged mean directly). 3 files, +39/-4 | 2 adopted as-is, 0 adopted with divergence work, 0 rejected | Zero textual conflicts. `SDTrainer.py`'s new hunk sits at the fft-loss tail of the loss-composition block, disjoint from the fork's `loss_sync_every`-gated block (~line 2270+); re-checked post-merge by grep that `neutralize_nonfinite_loss`/`DeferredLossTracker.push()` are still present and untouched. `toolkit/memory_management/manager.py` and `manager_modules.py` are not fork touchpoints (matches the 2026-09-02 sync's note that this file pair is upstream-owned). Fork surface unchanged at **57** modified upstream files | `npm ci`, `npx tsc --noEmit` **0 errors**, `npx next build` clean (all routes emitted), `npm test` **62 passed / 0 failed** (identical to baseline), `py_compile` on all 3 touched Python files: PASS, `scripts/verify_fork.py`: insertions/touchpoints/next-params/fork-docs/presets all OK. Repo-wide grep for conflict markers: none; working tree clean. Author/committer scan on the merge range: only `socrasteeze <socradeez@gmail.com>` (merge) and `Jaret Burkett <jaretburkett@gmail.com>` (upstream, preserved). **Not covered:** no `.venv`/torch in this Linux container (no `huggingface_hub` either), so `test_dataset_selection`, `test_fork_speed`, `test_ideogram4_prompt`, `test_lora_compile_scalars`, `test_presets` did not run — pre-existing gap, and the torchao offload fix is exactly the kind of change those would most want to exercise on real hardware (untested here beyond syntax); the cron worker's runtime paths remain untested as always |
| 2026-09-07 | 1 commit, `3e1cb5f` — DFE (diffusion feature extractor) rework for x0-prediction models: the vision-encoder/model gradient-checkpointing branches in `SDTrainer.py` now activate regardless of `train_config.gradient_checkpointing` (previously gated on it), and the DFE loss is now meaned and logged separately (`additional_logs['loss/dfe']`) before being weighted into `additional_loss`. 2 files, +56/-28 | 1 adopted as-is, 0 adopted with divergence work, 0 rejected | Zero textual conflicts. `SDTrainer.py`'s new hunk sits in the DFE-loading block (~line 403) and the loss-graph logging call (~line 750), both disjoint from the fork's `loss_sync_every`-gated block (~line 2270+); re-checked post-merge by diffing the file against `upstream/main` and confirming `neutralize_nonfinite_loss`/`DeferredLossTracker.push()` are still present and untouched. `toolkit/models/diffusion_feature_extraction.py` is not a fork touchpoint. Fork surface unchanged at **57** modified upstream files. **Note on this sync's own process:** this session started from a stale local `main` (last synced 2026-09-03) and had already fetched a much larger apparent window — `git rev-list --count HEAD..upstream/main` read 1099, mostly graph noise from a contributor PR merge splicing in an unrelated old commit chain (traced to a 2023 `Initial commit` reachable only through `fea99cf5`'s branch, content already matching our tree). That merge was built, gated, and fully green, but `git push origin main` was rejected non-fast-forward: two other syncs (2026-09-05, 2026-09-06, see rows below) had already landed on `origin/main` from a different session in the interim. The stale-base merge was abandoned on a backup branch (`backup-sync-20260907-stale-base`, not pushed), local `main` was hard-reset to the real `origin/main`, and the sync was redone from there — at which point the true incoming window collapsed to this single commit. Lesson for next time: fetch **and re-check `origin/main`**, not just `upstream`, before trusting a rev-list count — a concurrent sync session is a real possibility this fork doesn't otherwise guard against |`npm ci`, `npx tsc --noEmit` **0 errors** (app + worker projects, identical to baseline), `npx next build` clean, `npm test` **62 passed / 0 failed** (identical to baseline), `py_compile` on both touched Python files: PASS, plus the fork's own script suite: `test_dataset_selection` PASS, `test_ideogram4_prompt` PASS, `test_qol_scripts` PASS, `py_compile` on the fork scripts PASS. Repo-wide grep for conflict markers: none; working tree clean. Author/committer scan on the merge range: only `socrasteeze <socradeez@gmail.com>` (merge) and `Jaret Burkett <jaretburkett@gmail.com>` (upstream, preserved). **Not covered:** no `.venv`/torch in this container, so `test_fork_speed`, `test_lora_compile_scalars`, `test_presets` did not run — pre-existing gap, and this commit's DFE rework is exactly the kind of change those would most want to exercise (untested here beyond syntax); the cron worker's runtime paths remain untested as always |
| 2026-09-06 | 2 commits, `bbb1fda` through `7690ea6` — a gradient-accumulation scaling fix (`train_single_accumulation` now takes an `accum_scale` factor so summed micro-batch grads equal the mean over the effective batch instead of being summed unscaled) and a new opt-in `adamconvrot` optimizer (variable-precision Adam with stochastic rounding and accumulation). 3 files, +703/-2, plus new `toolkit/optimizers/adamconvrot.py` | 2 adopted as-is, 0 adopted with divergence work, 0 rejected | Zero textual conflicts; `extensions_built_in/sd_trainer/SDTrainer.py` auto-merged clean and was re-verified rather than assumed: the new `accum_scale` param and its use in `self.accelerator.backward(...)` sit entirely inside `train_single_accumulation`/`train_single_step`, disjoint from the fork's `loss_sync_every`-gated `neutralize_nonfinite_loss` call and the deferred `DeferredLossTracker.push()` (both still present and untouched, re-checked by grep after the merge, not assumed). `toolkit/optimizer.py` and the two new/touched optimizer files are not fork touchpoints. Fork surface unchanged at **57** modified upstream files | `npm ci`, `npx tsc --noEmit` **0 errors** (identical to baseline), `npx next build` clean (all routes emitted), `npm test` **62 passed / 0 failed** (identical to baseline), `py_compile` on all 4 touched/new Python files: PASS. Repo-wide grep for conflict markers: none; working tree clean. Author/committer scan: only `socrasteeze <socradeez@gmail.com>` (merge) and `Jaret Burkett <jaretburkett@gmail.com>` (upstream commits, preserved) on the merged range. **Not covered:** no `.venv`/torch in this container, so the four fork Python suites (`test_dataset_selection`, `test_fork_speed`, `test_ideogram4_prompt`, `test_lora_compile_scalars`) could not run — pre-existing gap in this Linux container, not merge-caused, and neither incoming commit touches any file those suites cover. The new `adamconvrot` optimizer and the accumulation-scaling fix are both GPU/runtime-bound and untested by any check available here; the cron worker's runtime paths remain untested as always |
| 2026-09-05 | 4 commits, `67e223b` through `fea99cf` (PR #813) — opt-in `pin_memory` on the CUDA DataLoader: a new `DatasetConfig.pin_memory` flag (default `False`) and a guarded `dataloader_kwargs['pin_memory'] = True` in `get_dataloader_from_datasets`. The default is off because page-locked RAM cannot be relocated by NVIDIA's Windows shared-memory VRAM-overflow fallback, which thrashes PCIe at the VRAM ceiling. 2 files (`toolkit/config_modules.py`, `toolkit/data_loader.py`), +16/-0 | 4 adopted as-is, 0 adopted with divergence work, 0 rejected | Zero textual conflicts, but **both** incoming files are fork touchpoints, so both were re-read rather than assumed. `config_modules.py`: upstream's `pin_memory` field landed in `DatasetConfig.__init__` beside `num_workers`/`prefetch_factor`, well away from the fork's Automagic fused-accumulation config-parse guard, which is intact. `data_loader.py`: upstream's new block sits in `get_dataloader_from_datasets` after the env-var defaults; the fork's `list_dataset_media_files` import and its hidden-dir-pruning replacement of upstream's raw `os.walk` are both untouched, as is `dataset_batch_size` selection. Fork surface unchanged at **57** modified upstream files — `scripts/verify_fork.py` re-derived it rather than trusting the count. Note for this box specifically: the new flag is off by default and should stay off unless VRAM headroom is stable, which is exactly the Windows case the upstream comment warns about | `scripts/verify_fork.py` all five checks OK (insertions, touchpoints, next-params, fork-docs, presets), `py_compile` on both touched files PASS, all four fork Python suites green against the repo `.venv` (`test_dataset_selection` 6, `test_fork_speed` 6, `test_ideogram4_prompt` 1, `test_lora_compile_scalars` 3 — 16 tests), `npx tsc --noEmit` **0 errors**, `npm test` **62 passed / 0 failed**. No conflict markers; working tree clean. `npx next build` was skipped: no file under `ui/` changed in this window, and `tsc` plus the contract suite already cover the unchanged UI. **Not covered:** the `pin_memory` path itself is runtime- and GPU-bound — nothing here exercises a real DataLoader against CUDA, and the flag is off by default so no current run changes behavior |
| 2026-09-03 (2nd) | 2 commits, `90515c5` through `b36bb39` — LTX 2 was handing the transformer hidden states in the wrong dtype (`ltx2.py` now casts to the transformer's own dtype on both the conditioning and the denoise path), plus a version bump. 2 files (`extensions_built_in/diffusion_models/ltx2/ltx2.py`, `version.py`), +3/-3 | 2 adopted as-is, 0 adopted with divergence work, 0 rejected | Zero textual conflicts. Neither incoming file is a fork touchpoint — `ltx2.py` and `version.py` are both upstream-owned and byte-identical to `upstream/main` after the merge — so no fork insertion point was in the blast radius. `scripts/verify_fork.py` (run as part of the gate script) confirms insertions, touchpoints, next-params, fork-docs and presets all OK. Fork surface unchanged at **57** modified upstream files | `pwsh scripts/run_fork_tests.ps1` full run on the operator's Windows box, all 12 gates PASS: `verify_fork`, the six fork Python suites (`test_dataset_selection`, `test_fork_speed`, `test_ideogram4_prompt`, `test_lora_compile_scalars`, `test_qol_scripts`, `test_presets`), `py_compile` on fork scripts, `npm test` (62 tests), `tsc --noEmit` (UI + worker), `next build`. `py_compile` additionally run by hand on both merged Python files: PASS. Working tree clean after the merge; no conflict markers. **Not covered:** the LTX 2 dtype fix itself is GPU/runtime-bound (it needs a real LTX 2 run to exercise) and no check available here touches it; the cron worker's runtime paths remain untested as always |
...
- `presets/flux2_klein_9b_character_lora.json`, `presets/flux2_klein_9b_style_lora.json` — the 9B tier (2026-08-24). Identical to the 4B presets except `arch` and `name_or_path`; previously the 9B was only reachable by hand-editing those two fields out of the 4B preset's description text. **Revised 2026-09-11**: Klein-specific guidance from BFL now exists and ratifies rank 16 / LR 1e-4 / 1500 starting steps, so these are no longer FLUX.1 extrapolations; the 9B style preset was raised to BFL's official 128/64 linear + 64/32 conv, and both 9B presets' "~32-48 GB VRAM" claim was corrected (the 9B is ~29 GB fp16 / ~15 GB fp8 — a single 24 GB card with quantization). Still not 9B-MEASURED, and the timestep type remains the one unsourced value
- `presets/krea2_character_lokr.json` — Krea 2 character **LoKr** variant (2026-09-11). `network.type: lokr` + `lokr_full_rank: true` + `lokr_factor: 8` and LR 5e-5 (half the LoRA presets'); otherwise `krea2_lora_low_vram` unchanged. Added because LoKr is reported to bleed identity far less than LoRA on Krea 2, which is the failure the existing presets answer with DOP/regularization. The factor is contested (community 4 / middle 8 / one published guide 16) and the direction is counter-intuitive — a LOWER factor is a LARGER network (`toolkit/models/lokr.py` `factorization()`). UNVERIFIED: no measured LoKr run in this fork. See PLAN.md "Klein/Krea 2 recipe review (2026-09-11)"
...
- `HANDOFF.md` — session state file for the next session (2026-09-11): branch, base, open items in order, decisions, traps, verify commands. Overwritten each delivery, never appended; `git log` is the history
- `ui/src/utils/batchAdvisor.ts` — machine-aware batch plan (2026-09-11). `tierForVramMb()` classes the live nvidia-smi `memory.total` into 16 GB / 24 GB / 32 GB+, `VRAM_TABLE` holds the OPERATOR'S MEASURED max `batch_size` per arch family per tier (desktop: SDXL/Anima 4, Klein/Krea 2 2 with 4 untested; laptop: SDXL/Anima 2, Krea 2 1, Klein OOM; 24 GB unmeasured, reuses the 16 GB floor, every cell flagged inferred), and `suggestBatch()` returns min(`maxHealthyBatch`, VRAM cell) routed as `batch_size` on the desktop and `gradient_accumulation` on 16/24 GB, with fused Automagic forced onto the batch_size route. Pure, no React. Tests: `ui/tests/batchAdvisor.test.mjs` (11, incl. a 21k-combo sweep asserting the plan never exceeds the dataset gate). Mounted in `StepSuggestion.tsx` as a Machine: select (auto-detected via `useGPUInfo`, overridable) + one "Apply batch + steps" button that writes `batch_size`, `gradient_accumulation`, the legacy `gradient_accumulation_steps` (to 1) and `steps` together. Update the table cells when a new batch/VRAM point is measured — that is the whole value of it
...
- `presets/` — preset config files (drop-in JSON/YAML). 2026-07-19: seven LDS-ported presets added (zimage char/style/concept, flux2_klein char/style, krea2 concept, sdxl concept) + `flux_lora_24gb.json` v1.1 EMA fidelity fix; provenance table in `presets/README.md`, comparison in `docs/preset_alignment_2026_07.md` (fork-only). 2026-07-21: `flux2_klein_style_lora.json` re-tuned to 64/32 linear + 32/16 conv (a half-scale fold of LDS's researched 128/64/64/32; see the doc's 2026-07-21 update). 2026-09-11: Klein + Krea 2 recipe review — all five `flux2_klein_*` presets to v1.2 (character steps 2500 -> 1500 per BFL's published starting point, 9B style network raised to the official 128/64/64/32, wrong VRAM claims corrected on all four 9B/4B descriptions), `presets/krea2_character_lokr.json` added, `ARCH_HEURISTICS` gained explicit `flux2_klein_4b`/`9b` entries (steps/item unchanged at 60, ceiling 3000 -> 2500), and `forkDocs.tsx`'s `network.lokr_factor` description was corrected — it had the factor direction backwards. Same-day addendum: the Klein/Krea 2 recipes gained published optimizer, batch, dataset-size and repeat-count guidance, including a shared `REPEATS_NOTE` const (kohya/musubi repeat advice does not transfer to a step-bounded trainer) and the report that adafactor fails on Klein 9B character training. A second addendum re-read `toolkit/optimizers/automagic3.py` (current since the 2026-08-14 sync) and corrected the Krea 2 notes + `krea2_lora_16gb` description: the min_lr/max_lr bounds are optional user rails whose defaults are overflow guards, runaway LR is v2's documented flaw that v3 fixes structurally, this fork deviates from three author defaults (launch LR, weight decay, the rails), and `polarity_history` (default 8, range 2-64) is an undocumented second dial. No preset VALUE changed. A third addendum added a shared `EFFECTIVE_BATCH_NOTE` const (batch_size vs gradient_accumulation: same gradient, different hardware — 32 GB desktop profiles use batch, 16 GB profiles use accumulation, and accumulating pays a `torch.cuda.empty_cache()` per micro-batch under `low_vram`) and corrected `presets/anima_lora_automagic.json` to v1.2, whose description claimed its parent's accumulation 2 while the file pins 1 (fused Automagic cannot accumulate, so it trains at effective batch 1). A fourth addendum resolved the laptop-tier effective-batch question against the presets' favour: every preset matches its own arch's advisor batch recommendation (SDXL family 2, everything else 1), so `flux_lora_laptop16gb` at effective 1 is correct; the wrong text was `docs/profiles.md`'s bold "every recipe value is inherited unchanged" (contradicted by its own lever 4) and the matching `presets/README.md` row, both now naming the SDXL-family effective-batch exception. See PLAN.md. 2026-08-29: the three `zimage_*` presets (v1.1) moved from `arch: zimage` to `arch: zimage:turbo` + `assistant_lora_path` — v1.0 had paired the Turbo weights with the base arch and no adapter, i.e. trained the distilled model directly (the trainer strips the `:variant`, `toolkit/config_modules.py`)
