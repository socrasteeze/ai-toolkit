...
- `testing/test_fork_speed.py`, `testing/test_ideogram4_prompt.py` — CPU-only regression tests for the fork speed helpers and the Ideogram prompt's literal backslash examples
...
- `ui/src/server/datasetFiles.ts` — dataset scans/subfolder resolution; stably re-exports the top-level helpers from `datasetPath.ts` so existing route imports do not churn
