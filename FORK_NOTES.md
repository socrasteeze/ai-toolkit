...
|---|---|---|---|---|
| 2026-08-14 | 1 commit, `5261d3f` (Add checkpointing to the Wan2.1 encoder) | 1 adopted as-is, 0 rejected | None — the single incoming file, `toolkit/models/wan21/autoencoder_kl_wan.py`, is upstream-owned and not a fork touchpoint (`git diff $(git merge-base HEAD upstream/main)..HEAD` reports nothing under `toolkit/models/wan21/`); the merge was conflict-free and additive (+45 lines: a cache-free `WanResample.forward` for `downsample3d` plus an `AutoencoderKLWan._encode` override that runs cache-free when gradient checkpointing is active) | `npm ci`, `npx tsc --noEmit` **0 errors** (identical to baseline), `npx next build` clean with all fork routes present (`/api/presets`, `/api/presets/[name]`), `py_compile` on the touched file plus `compileall` over `toolkit/`: PASS. Contract suites: **29 Node passed**, identical to baseline. The Python suites could not run at all in this container — neither `torch` nor `huggingface_hub` is installed, and `toolkit/__init__.py` imports the latter at module load — so all 13 Python cases are uncovered this window; none of them touch the incoming file. Fork surface re-derived after the merge: still exactly the **57** modified upstream files this table records. `ui/package-lock.json` byte-identical to upstream; `ui/package.json` carries the fork's one-line `test` script (pre-existing, predates this window) |
