...
async function zipDataset(datasetName: string, captionsOnly: boolean) {
  ...
  // realpath resolves symlinks; re-check containment so a dataset symlink
  // cannot make the archive reader walk outside DATASETS_FOLDER.
  const relativeFolder = path.relative(datasetsRoot, folderPath);
  if (
    relativeFolder === '' ||
    relativeFolder === '..' ||
